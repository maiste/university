...
import trac.javix.{AST => T, Interp}
import trac.fopix.{AST => S, Parse}
import trac.Invalid

class JavixTestSuite extends AnyFunSuite {
    ...
    ast.foreach {
      ast => {
        failAfter(5 seconds) { // In case of timeout, fail
          info("Compile")
          val javixAst = Fopix2Javix.compile(file, ast)
          try {
            info("Interpret")
            val javixEval = Interp.eval(javixAst, false)
            info("Assert")
