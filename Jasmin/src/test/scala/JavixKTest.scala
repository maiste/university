...
import trac.javix.{AST => U, Interp}
import trac.Invalid

class JavixKontTestSuite extends AnyFunSuite {
      ...
      ast.foreach {
        ast => {
          failAfter(5 seconds) { // In case of timeout, fail
            ...
            val javixAst = Kontix2Javix.compile("myTest", kontixAst)

            try {
              info("Interpret")
              val javixEval = Interp.eval(javixAst)
              info("Assert")
              assert(javixEval == res)
            } catch {
