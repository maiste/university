 ...
 *<br>
 * Cet IA est un milieu entre la stratégie Hard_v2 et Hard_v3.
 * Il va chercher à maximiser les pièces qu'il pose tout en minimisant
 * les pièces que peut poser le meilleur joueur.
 *
 * @author Blokus_1
 */
public class Hard_v4 implements Strategy{

    private Blokus game;
    private Blokus copieGame;
    private LinkedList<Message>[] memoires = new LinkedList[2];
    private Player player;

    public Hard_v4(Blokus game) {
        this.game = game;
    ...
    /**
     * On met à jour la liste des possibilités du meilleur des autres joueurs.
     */
    public void updateListOfMessageForOther(){
        for (int i = 0; i < memoires.length-1; i++) {
            for(LinkedList<Message> messages : ((IAPlayer)player).possibleStrike(getStrongerPlayer())){
                memoires[i+1].addAll(messages);
            }
        }
        game.setPlayerTurn(player.getId()-1);
    }

    /**
     * Déterminer quel est le meilleur des joueurs dans l'état actuel.
     * @return le meilleur des joueurs différents de l'IA
     */
    public Player getStrongerPlayer(){
        Player stronger = (game.getPlayersByScore()[0] != player)? game.getPlayersByScore()[0] : game.getPlayersByScore()[1];
        game.setPlayerTurn(stronger.getId()-1);
        return stronger;
    }
}